...
print "*******************************************************************************\n";
print "**                       Welcome to correlation scan (v0.05).                **\n";
print "**---------------------------------------------------------------------------**\n";
...
sub create_res_file {
    ...
    for (my $i = 0; $i < @$contents; $i++) {
        ...
        my $_pavg       = $pavg->[$i];
        print $OUT join("\t", (@{$contents->[$i]}, $_adj_pvalue)), "\n";
    }
...
sub create_chc_file {
    ...
    foreach my $chr (sort { $a <=> $b } keys %{$windows_per_chr}) {
...
print "\n\nOutput files created as \"$out.res & $out.chc ... done ";
exit;
...
sub die_and_exit {
    ...
    my $help = $message;
    $help .= "\nUsage:\n\n perl correlation_scan.pl <options>\n\n";
    $help .= "       --file=<input file>           Expects tab separated ascii file.\n";
    $help .= "       --size=<window size>          Default size is 500.\n";
    $help .= "       --slide=<sliding window size> Default size is 100.\n";
